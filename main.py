...
# Session state to track if URLs have been processed
...
if st.sidebar.button("Process URLs"):
    if not urls:
        ...
        st.session_state.urls_processed = False  # Reset before processing
        status_text = ""
        for status in process_urls(urls):
            status_text += status + "\n"
            status_placeholder.text(status_text)  # update in real-time
        st.session_state.urls_processed = True  # Mark as processed
...
# Display hint if question box is disabled
